...
\hline\hline
$S_j$ & Susceptible host density for species $j$  &$individuals \cdot L ^{-1}$\\
$I_j$ & Infected host density for species $j$ & $ individuals \cdot L ^{-1}$\\
$F$& Alga density& $10^5 cells \cdot L ^{-1}$\\
$P$ & Spore density & $10^3 spores \cdot L ^{-1}$\\
$K_F$ & Alga carrying capacity&$10^5 cells \cdot L ^{-1} $ \\
$r_n$ & Native growth rate &$10^{-5} \cdot cells^{-1} \cdot day^{-1}$\\
$r_i$ & Invasive growth rate &$10^{-5} \cdot cells^{-1} \cdot day^{-1}$\\
$\theta_{I_n}$ & Native infected host mortality rate&$ day^{-1}$ \\
$\theta_{I_i}$ & Invasive infected host mortality rate&$ day^{-1}$ \\
$\theta_{S_i}$ & Invasive susceptible host mortality rate&$ day^{-1}$ \\
$\theta_{S_n}$ & Native susceptible host mortality rate&$ day^{-1}$ \\
$\theta_{P}$ & Spore degradation rate&$ day^{-1}$ \\
$p_n$ &Native probability of infection per spore &$10^{-3} \cdot individuals \cdot spore^{-1} $ \\
$p_i$ & Invasive probability of infection per spore&$10^{-3} \cdot individuals \cdot spore^{-1} $ \\
$f_{S_n}$ & Native susceptible host filtering rate&$L \cdot day^{-1} \cdot individuals^{-1} $ \\
$f_{S_i}$ & Invasive susceptible host filtering rate&$L \cdot day^{-1} \cdot individuals^{-1} $ \\
$f_{I_n}$ & Native infected host filtering rate&$L \cdot day^{-1} \cdot individuals^{-1} $ \\
$f_{I_i}$ & Invasive infected host filtering rate&$L \cdot day^{-1} \cdot individuals^{-1} $ \\
$\delta$ & Sampling rate&$day ^{-1} $ \\
$\alpha$ & Alga maximum exponential growth rate&$day ^{-1} $ \\
$\gamma_n$ & Native host consumption rate&$L \cdot inidviduals^{-1} \cdot day ^{-1} $ \\
