...
As juveniles rarely exhibit clear signs of infection, only adult \textit{Daphnia} were used for quantifying effects in both Non-parasitized and Parasitized Dynamics to minimize information bias. Figures below illustrates these dynamics, with the figure 1 depicting Non-parasitized Dynamics (mixed-species) where changes in native and invasive host densities reflect direct resource competition, for example, food alga \textit{Ankistrodesmus falcatus}. Figure 2 shows Parasitized Dynamics, where the interplay of susceptible and infected \textit{Daphnia} of different species is further influenced by parasites, complicating the variation of susceptible and infected \textit{Daphnia} of different races, which makes it difficult to directly analyze the sources of influence on their variation. Non-parasitized Dynamics will be analyzed at the first place to help understand the effect of resource competition in mixed-species. And based on this result, the correlation between native host and invasive host in Parasitized Dynamics will be further explored.

\begin{figure}[H]
\centering
<<echo=FALSE, fig.cap="Plot1", fig.align='center', fig=TRUE, width=16, height=10>>=
...
# Combine parasitized data with trial information
...
# Assuming 'mixed_nopara_data_combined' is already your data frame as shown
# Create a mapping from Trial letters to bucket numbers
trial_to_bucket <- function(trial) {
  # Extract the letter from the trial name
  letter <- substr(trial, 7, 7)
  # Convert the letter to a bucket number (A=1, B=2, ..., J=10)
  bucket_number <- match(letter, LETTERS[1:26])
  # Return the bucket number prefixed with "Bucket"
  return(paste("Bucket", bucket_number))
}

# Apply the function to create a new 'Bucket' column
mixed_nopara_data_combined$Bucket <- sapply(mixed_nopara_data_combined$Trial, trial_to_bucket)
mixed_para_data_combined$Bucket <- sapply(mixed_para_data_combined$Trial, trial_to_bucket)
p1 <- ggplot(mixed_nopara_data_combined, aes(x = day)) +
  geom_line(aes(y = dent.adult, group = interaction(Bucket, "dent.adult"), color = "Dent Adult"), size = 1) +  # Map color to a factor for legend
  geom_line(aes(y = lum.adult, group = interaction(Bucket, "lum.adult"), color = "Lum Adult"), size = 1) +  # Map color to a factor for legend
  scale_x_continuous(limits = c(0, 52)) +
  scale_y_continuous(limits = c(0, 175)) +
  facet_grid(. ~ Bucket, scales = "fixed", space = "free") +
  theme_minimal() +
  labs(title = "Daphnia Density in Non-parasitized Dynamics",
       color = "Species") +
  scale_color_manual(values = c("Dent Adult" = "black", "Lum Adult" = "blue"),
                     labels = c("Dent Adult" = "Dent adult", "Lum Adult" = "Lum adult")) +
  theme(legend.position = "right")  # Adjust the legend position as needed

p2 <- ggplot(mixed_para_data_combined, aes(x = day)) +
  geom_line(aes(y = dentadult, color = "Dent Adult", group = interaction(Bucket, "dentadult"))) +  # Added color mapping for dentadult
  geom_line(aes(y = lumadult, color = "Lum Adult", group = interaction(Bucket, "lumadult"))) +  # Ensure consistent color mapping
  geom_line(aes(y = luminf, color = "Lum Inf", group = interaction(Bucket, "luminf"))) +  # Ensure consistent color mapping
  geom_line(aes(y = dentinf, color = "Dent Inf", group = interaction(Bucket, "dentinf"))) +  # Ensure consistent color mapping
  scale_x_continuous(limits = c(0, 52)) +
  scale_y_continuous(limits = c(0, 175)) +
  facet_grid(. ~ Bucket, scales = "fixed", space = "free") +
  theme_minimal() +
  ...
  scale_color_manual(values = c("Dent Adult" = "black", "Lum Adult" = "blue", "Lum Inf" = "green", "Dent Inf" = "red"),
                     labels = c("Dent Adult" = "Dent adult", "Lum Adult" = "Lum adult", "Lum Inf" = "Lum inf", "Dent Inf" = "Dent inf")) +
  guides(color = guide_legend(title = "Species"))  # Ensure the legend is properly configured

# Arrange plots vertically
grid.arrange(p1, p2, ncol = 1)

@
\caption{Plot of density of hosts in dynamics including both native and invasive host species of \textit{Daphnia}. The trail A to J represents different buckets in different experiment. The black lines represent the \textit{D. dentifera} density, the red lines show the \textit{D. lumholtzi} density, the green lines are the infected \textit{D. dentifera} density, and the blue lines indicate the infected \textit{D. lumholtzi} density.}
\end{figure}
...
\end{center}
\vspace{-5mm}
\caption{A flow diagram for the SIRPF2 model. Apart from the quantified states above, the graph includes $R$ state, which represents deadth status. The flow diagram shows the how two species of \textit{Daphnia} interact with each other indirectly through $F$ and $P$. The marginal processes have been shown in SIRPF mode, and SIRPF2 model is the adaption form in mixed-species dynamcis}
...
parms$f_Sn = parms$f_Sn * 10000
parms$f_Si = parms$f_Si * 10000
parms$f_In = parms$f_In * 1000
parms$f_Ii = parms$f_Ii * 1000000
parms$alpha = parms$alpha * 10000
parms$gamman = parms$gamman * 1e8
parms$gammai = parms$gammai * 1e4

parms$sigSi = parms$sigSi * 1e4
parms$sigIn = parms$sigIn * 1e5
parms$sigIi = parms$sigIi * 1e5
parms$sigF = parms$sigF * 10
parms$sigP = parms$sigP * 100
@
...
$K_F$ & Alga carrying capacity&$10^2\cdot cells \cdot L ^{-1} $ & \Sexpr{mysignif(parms$K_F,2)}\\
$r_n$ & Native growth rate &$10^{-6} \cdot cells^{-1} \cdot day^{-1}$ & \Sexpr{mysignif(parms$rn,2)}\\
$r_i$ & Invasive growth rate &$10^{-6} \cdot cells^{-1} \cdot day^{-1}$ & \Sexpr{mysignif(parms$ri,2)}\\
$\theta_{I_n}$ & Native infected host mortality rate&$ 10^{-1} \cdot day^{-1}$  & \Sexpr{mysignif(parms$theta_In,2)}\\
$\theta_{I_i}$ & Invasive infected host mortality rate&$10^{-1} \cdot day^{-1}$  & \Sexpr{mysignif(parms$theta_Ii,2)}\\
$\theta_{S_i}$ & Invasive susceptible host mortality rate&$10^{-1} \cdot day^{-1}$ & \Sexpr{mysignif(parms$theta_Si,2)}\\
$\theta_{S_n}$ & Native susceptible host mortality rate&$10^{-1} \cdot day^{-1}$ & \Sexpr{mysignif(parms$theta_Sn,2)}\\
$\theta_{P}$ & Spore degradation rate&$10^{-1} \cdot  day^{-3}$ & \Sexpr{mysignif(parms$theta_P,2)}\\
$p_n$ &Native probability of infection per spore &$10^{-4} \cdot individuals \cdot spore^{-1} $ & \Sexpr{mysignif(parms$probn,2)}\\
$p_i$ & Invasive probability of infection per spore&$10^{-4} \cdot individuals \cdot spore^{-1} $ & \Sexpr{mysignif(parms$probi,2)}\\
$f_{S_n}$ & Native susceptible host filtering rate&$10^{-4} \cdot L \cdot day^{-1} \cdot individuals^{-1} $ & \Sexpr{mysignif(parms$f_Sn,2)}\\
$f_{S_i}$ & Invasive susceptible host filtering rate&$10^{-4} \cdot L \cdot day^{-1} \cdot individuals^{-1} $& \Sexpr{mysignif(parms$f_Si,2)} \\
$f_{I_n}$ & Native infected host filtering rate&$10^{-3} \cdot L \cdot day^{-1} \cdot individuals^{-1} $& \Sexpr{mysignif(parms$f_In,2)} \\
$f_{I_i}$ & Invasive infected host filtering rate&$10^{-6} \cdot L \cdot day^{-1} \cdot individuals^{-1} $ & \Sexpr{mysignif(parms$f_Ii,2)}\\
$\delta$ & Sampling rate&$day ^{-1} $ & 0.013\\
$\alpha$ & Alga maximum exponential growth rate&$10^{-4} \cdot day ^{-1} $& \Sexpr{mysignif(parms$alpha,2)} \\
$\gamma_n$ & Native host consumption rate&$10^{-4} \cdot L \cdot individual^{-1} \cdot day ^{-1} $ & \Sexpr{mysignif(parms$gamman,2)}\\
$\gamma_i$ & Invsaive host consumption rate&$10^{-4} \cdot L \cdot individual^{-1} \cdot day ^{-1} $ & \Sexpr{mysignif(parms$gammai,2)}\\
...
$\sigma_{S_i}$ & Standard deviation of Brownian motion of susceptible invasive &$10^{-4} \cdot \sqrt{individual \cdot t^{-1}}$ & \Sexpr{mysignif(parms$sigSi,2)}\\
$\sigma_{I_n}$ & Standard deviation of Brownian motion of infected native&$10^{-5} \cdot \sqrt{individual \cdot t^{-1}}$ & \Sexpr{mysignif(parms$sigIn,2)}\\
$\sigma_{I_i}$ & Standard deviation of Brownian motion of infected invasive&$10^{-5} \cdot \sqrt{individual \cdot t^{-1}}$ & \Sexpr{mysignif(parms$sigIi,2)}\\
$\sigma_{F}$ & Standard deviation of Brownian motion of alga&$10^{-1} \cdot \sqrt{individual \cdot t^{-1}}$ & \Sexpr{mysignif(parms$sigF,2)}\\
$\sigma_{P}$ &Standard deviation of Brownian motion of parasite &$10^{-2} \cdot \sqrt{individual \cdot t^{-1}}$ & \Sexpr{mysignif(parms$sigP,2)}\\
...
&dF(t)=\alpha F(t)\left(1-\frac{F(t)}{K_F}\right)\cdot dt-\gamma_n F(t) S_n(t) \cdot dt - \gamma_i F(t) S_i(t) \cdot dt -\delta F(t) \cdot dt+ \mu \cdot dt +F(t) d \zeta_F.\\
...
<<Setup, include=FALSE,echo=FALSE,results=hide>>=
 ...
 save(SF_invasive_estimate=SF_invasive_estimate,SF_native_estimate=SF_native_estimate,SF2_estimate=SF2_estimate,SIRPF_invasive_estimate=SIRPF_invasive_estimate,SIRPF_native_estimate=SIRPF_native_estimate,SIRPF2_estimate=SIRPF2_estimate,file = "SIRPF_model_estimate.RData")
@
