...
Ecological experiments contain many elements or processes that are difficult to observe, so models that can describe both observable and unobservable processes are needed to explain the entire ecological experiment. As mathematical descriptions of the elements that make up a system, mechanistic models can describe the interactions between the elements of the system and their interactions with the environment. Using established processes, they can elucidate the role of hidden mechanisms \citep{Duarte2003}. The purpose of mechanistic modeling in ecology is to use mathematical models to describe the development and interactions of dynamic systems. This quantitative tool allows researchers to assess the explanatory potential of various conceptual models and to measure the magnitude of relevant parameters. Mechanistic modeling has been widely used in the social sciences, industrial engineering and physics. Now, increased computational power has made it even more effective. Because of the non-smooth, non-linear and stochastic nature of ecological information, traditional time-series analysis offers limited explanatory power. Despite creating theoretical models for the data, linking the theory to the data remains a challenging task. As a result, eco-mechanical modeling is unable to address most problems on a large scale, but only a limited range of problems \citep{levin1992problem}.\\
...
p1 <- ggplot(mixed_nopara_data_combined, aes(x = day, y = sqrt(dent.adult))) +
  geom_line(aes(color = "Dent Adult"), size = 1) +
  geom_line(aes(y = sqrt(lum.adult), color = "Lum Adult"), size = 1) +
...
p2 <- ggplot(mixed_para_data_combined, aes(x = day, y = sqrt(dentadult))) +
  geom_line(aes(color = "Dent Adult"), size = 1) +
  geom_line(aes(y = sqrt(lumadult), color = "Lum Adult"), size = 1) +
  geom_line(aes(y = sqrt(luminf), color = "Lum Inf"), size = 1) +
  geom_line(aes(y = sqrt(dentinf), color = "Dent Inf"), size = 1) +
  facet_wrap(~Bucket, nrow = 1, labeller = bucket_labeller) +
  ...
  scale_color_manual(values = c("Dent Adult" = "black", "Lum Adult" = "blue", "Lum Inf" = "green", "Dent Inf" = "red")) +
  theme_minimal() +
  guides(color = guide_legend(title = "Species"))
...
\end{center}
\vspace{-5mm}
\caption{A flow diagram for the SIRPF2 model. Apart from the quantified states above, the graph includes $R$ state, which represents death status. The flow diagram shows the how two species of \textit{Daphnia} interact with each other indirectly through $F$ and $P$. The marginal processes have been shown in SIRPF mode, and SIRPF2 model is the adaption form in mixed-species dynamics}
...
In the ecological study of \textit{Daphnia} populations, we employ the negative binomial distribution to address the inherent overdispersion observed in population counts. This distribution effectively models count data where the variance surpasses the mean, typical in ecological data due to the correlated nature of biological events \citep{ning2021systemic}. In our model, the parameter $F$ represents the number of successful \textit{Daphnia} reproductions before observing a fixed number of food scarcity events, and $S / \delta$ reflects average number of \textit{Daphnia} per liter per sampling time in each trial. This framework aligns with the observed dynamics in \textit{Daphnia} populations, where variability in reproduction rates, influenced by fluctuating food availability, leads to overdispersed population counts. The negative binomial distribution, therefore, provides a robust statistical tool to assess population dynamics, accommodating the irregularities in reproduction and survival patterns shaped by environmental constraints.\\

The model's effectiveness in capturing the \textit{Daphnia} population dynamics hinges on its ability to account for overdispersion, a critical factor often overlooked in simpler models like the Poisson distribution \citep{breto2011compound}. As such, the negative binomial model stands as a more accurate representation of the complex ecological processes governing \textit{Daphnia} populations, particularly under varying food conditions.
