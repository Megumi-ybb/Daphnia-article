...
$\gamma_n$ & Native host consumption rate&$10^{-4} \cdot L \cdot inidviduals^{-1} \cdot day ^{-1} $ & \Sexpr{mysignif(parms$gamman,2)}\\
$\gamma_i$ & Invsaive host consumption rate&$10^{-4} \cdot L \cdot inidviduals^{-1} \cdot day ^{-1} $ & \Sexpr{mysignif(parms$gammai,2)}\\
...
&dF(t)=\alpha F(t)\left(1-\frac{F(t)}{K_F}\right)\cdot dt-\gamma_n F(t) S_n \cdot dt - \gamma_i F S_i \cdot dt -\delta F(t) \cdot dt+ \mu \cdot dt +F(t) d \zeta_F.\\
